...
bool BMP::create(Bmp& bmp, int width, int height) {
...
ReadResult BMP::read(Bmp& bmp, const std::string& filename) {
...
bool BMP::write(Bmp& bmp, const std::string& filename) {
...
int BMP::calcBmpPadding(int width) {
    const int bytesInRow = width * sizeof(Pixel);
    return bytesInRow % 4 ? 4 - bytesInRow % 4 : 0;
}

Pixel& BMP::getFilePixel(const Bmp& bmp, int row, int col) {
...
void BMP::fileDataToVector(Bmp& bmp) {
...
void BMP::vectorToFileData(Bmp& bmp) {
    for (int row = 0; row < bmp.infoHdr.height; ++row) {
        for (int col = 0; col < bmp.infoHdr.width; ++col) {
